...
-- private: owner only · unlisted: anyone who has the (unguessable) id = invite link · public: listed only when approved
create policy "owner reads own"                on public.creations for select using (auth.uid() = owner);
create policy "unlisted or approved-public readable" on public.creations for select
  using (visibility = 'unlisted' or (visibility = 'public' and approved = true));
